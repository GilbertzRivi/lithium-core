...
use crate::{crypto::{aead, keys}, error::Result, keys::{KeyManager, MkProvider}, labels::{DB_DEK_LABEL, USERS_UUID_NAMESPACE_LABEL}, secrets::Byte32, secrets::bytes::SecretBytes};
...
impl<P: MkProvider + Send + Sync + 'static> DataManager<P> {
    ...
    pub async fn users_uuid_namespace(&self) -> Result<Uuid> {
        let d = self.key_manager.lock().await.derive_secret32(USERS_UUID_NAMESPACE_LABEL)?;
        let mut b = [0u8; 16];
